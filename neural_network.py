...
def label_leaves(lung):
    ...
    if lungtype == 'B': ans = [1,0,0,0]
    elif lungtype == 'C': ans = [0,1,0,0]
    elif lungtype == 'R': ans = [0,0,1,0]
    elif lungtype == 'S': ans = [0,0,0,1]
...
def main():
    ...
    model_info = model.fit({'input': X}, {'targets': Y}, n_epoch=40, validation_set=({'input': test_x}, {'targets': test_y}), snapshot_step=40, show_metric=True, run_id=MODEL_NAME)
